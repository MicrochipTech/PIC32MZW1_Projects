...
/*SSID name for directed scan*/
#define SSID_UNDER_SCAN     "DEMO_AP"
// *****************************************************************************
// *****************************************************************************
// Section: Global Data
// *****************************************************************************
...
) 
{
    ...
    else 
    {
        ...
        SYS_CONSOLE_PRINT(" %02d %02X:%02X:%02X:%02X:%02X:%02X %s\r\n", pBSSInfo->ctx.channel,
         ...
         if(strncmp((char *)pBSSInfo->ctx.ssid.name,SSID_UNDER_SCAN,strlen(SSID_UNDER_SCAN)) == 0)
	{
             ssid_found = true;
	}
         if((ofTotal > 0 ) && (ofTotal == index))
         {
             if(ssid_found == true) 
             {
                 SYS_CONSOLE_MESSAGE("SSID found\r\n");
             }
             else
             {
                 SYS_CONSOLE_MESSAGE("SSID not found\r\n");
             }
         }
}
}
